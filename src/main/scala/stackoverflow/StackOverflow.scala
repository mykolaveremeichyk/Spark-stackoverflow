...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    ...
    val answers: RDD[(QID, Answer)] = postings.filter(v => v.postingType == 2).map(a => (a.parentId.get, a))
    val joinedRdd: RDD[(QID, (Question, Answer))] = questions.join(answers)
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map(raw => (raw._2.head._1, answerHighScore(raw._2.map(_._2).toArray)))
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    val middleRDD: RDD[(Option[Int], HighScore)] = scored.map { tuple => {
      val firstLang = firstLangInTag(tuple._1.tags, langs)
      (firstLang, tuple._2)
    }
    }
    middleRDD.filter(_._1.isDefined).map(tuple => (tuple._1.get * langSpread, tuple._2))
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = calcNewMeansMap(means, vectors)
  ...
  def calcNewMeansMap(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): scala.collection.Map[(Int, Int), (Int, Int)] =
    val vectorDistances: RDD[((Int, Int), (Int, Int))] =
      vectors
        .map(vector => (vector, means.map(mean => (mean, distance(mean, vector))).minBy(_._2)._1))
    assert(vectorDistances.count() == vectors.count(), "Incorrect number of calculated vectors and distances")

    val clusters: RDD[((Int, Int), Iterable[(Int, Int)])] = vectorDistances.map(v => (v._2, v._1)).groupByKey()
    assert(clusters.count() == means.length, "Incorrect number of calculated clusters")

    clusters.map(element => (element._1, calculateNewCoordinate(element._2))).collectAsMap()

  def distance(p1: (Int, Int), p2: (Int, Int)): Double =
    sqrt((p1._1 - p2._1) * (p1._1 - p2._1) + (p1._2 - p2._2) * (p1._2 - p2._2))

  def calculateNewCoordinate(vectors: Iterable[(Int, Int)]): (Int, Int) =

    val size = vectors.size
    val sumX = vectors.map(v => v._1).foldLeft(0L)(_ + _)
    val sumY = vectors.map(v => v._2).foldLeft(0L)(_ + _)
    val newPoint = (sumX / size, sumY / size)
    (newPoint._1.toInt, newPoint._2.toInt)
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double = vs.groupBy(_._1).maxBy(_._2.size)._2.size * 100/vs.size // percent of the questions in the most common language
      val clusterSize: Int = vs.size
      val medianScore: Int = vs.map(v => v._2).sum/vs.size
